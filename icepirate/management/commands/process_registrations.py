...
class Command(BaseCommand):
    ...
    def check_names(self, input_name, national_name):
                    ...
                    if member_try.count() > 0:
                        ...
                        if reg_check.count() == 0:
                            ...
                            if national['is_valid'] and national['is_individual']:
                                ...
                                stdout.write('- Checking if names match ("%s" and "%s")...' % (reg['name'], national['name']))
                                stdout.flush()
                                if self.check_names(reg['name'], national['name']):
                                    stdout.write(' yes\n')
                                else:
                                    stdout.write(' no\n')
                                ...
                                member.legal_name = national['name']
                                member.legal_address = national['legal_address']
                                member.legal_zip_code = national['legal_zip_code']
                                member.legal_zone = national['legal_zone']
